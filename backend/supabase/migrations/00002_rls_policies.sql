-- ==============================================================================
-- CAREGRID: Migration 00002 - Row-Level Security (RLS) Policies (Revised MVP)
...
CREATE POLICY facilities_admin_manage ON facilities
    FOR ALL TO authenticated USING (current_user_role() IN ('district_officer', 'state_admin'));
...
CREATE POLICY patients_health_officer_access ON patients
    FOR SELECT TO authenticated USING (current_user_role() IN ('district_officer', 'state_admin'));
...
CREATE POLICY triage_clinician_access ON triage_assessments
    FOR ALL TO authenticated
    USING (current_user_role() IN ('asha_worker', 'anm_worker', 'medical_officer', 'specialist_doctor', 'facility_admin'));
...
CREATE POLICY appointments_facility_manage ON appointments
    FOR ALL TO authenticated
    USING (
        current_user_role() IN ('medical_officer', 'specialist_doctor', 'facility_admin')
...
-- 8. Follow-up Tasks
CREATE POLICY follow_up_asha_access ON follow_up_tasks
    FOR ALL TO authenticated
    USING (
        assigned_asha_id = auth.uid()
        OR current_user_role() IN ('medical_officer', 'facility_admin', 'district_officer', 'state_admin')
    );

CREATE POLICY follow_up_patient_read ON follow_up_tasks
    FOR SELECT TO authenticated USING (patient_id = auth.uid());
